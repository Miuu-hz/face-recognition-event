...
**แก้ไขไฟล์ .env:**
```bash
# เปิดด้วย text editor ที่ชอบ
nano .env
# หรือ
code .env
```

**ค่าที่ต้องตั้ง (ขั้นต่ำ):**
```env
# CRITICAL: เปลี่ยน SECRET_KEY!
...
# Google OAuth (ตั้งค่าในขั้นตอนถัดไป)
GOOGLE_CLIENT_SECRETS=client_secrets.json
```

**สร้าง SECRET_KEY แบบปลอดภัย:**
```bash
...
**หมายเหตุ:**
- ✅ ไฟล์ .env จะถูก ignore โดย git อัตโนมัติ
- ⚠️ อย่า commit SECRET_KEY หรือ credentials ลง git
- 📝 ดู .env.example สำหรับตัวเลือกทั้งหมด

### ขั้นตอนที่ 5: ติดตั้ง Localtunnel
```bash
npm install -g localtunnel
```

### ขั้นตอนที่ 6: เตรียม Database
```bash
...
**หมายเหตุ:**
- ✅ ไฟล์นี้ถูก ignore โดย .gitignore อัตโนมัติ
- ⚠️ อย่า commit ไฟล์นี้ลง git (มี client_secret!)
- 📝 ถ้าใช้ชื่อไฟล์อื่น ให้แก้ไขใน `.env`:
  ```env
  GOOGLE_CLIENT_SECRETS=path/to/your/file.json
  ```
